...
import com.ArtGalleryManagement.Backend.Repository.HistoryRepository;
...
public class ProductServiceImpl implements ProductService {
	...
	public ProductServiceImpl(ProductRepository productRepository, CheckOutRepository checkoutRespository,HistoryRepository historyRepository) {
		super();
	...
	public void returnProduct(String userEmail, int productId) throws Exception {
		...
		History history=new History(
				userEmail,
				validateCheckout.getCheckout_date(),
	...
	@Override
	public void renewLoan(String userEmail,int productId) throws Exception{
		CheckOut validateCheckout=checkoutRepository.findByUserEmailAndProductId(userEmail, productId);
		if(validateCheckout==null) {
			throw new Exception("Product does not exist or not checked out by the user");
		}
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
		Date d1=sdf.parse(validateCheckout.getReturn_date());
		Date d2=sdf.parse(LocalDate.now().toString());
		if(d1.compareTo(d2)>0 || d1.compareTo(d2)==0) {
			validateCheckout.setReturn_date(LocalDate.now().plusDays(7).toString());
